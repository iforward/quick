 ...
 *
 * A Logger instance can be accessed via `Q::getLogger()`. You can call the method [[log()]] to record a single log message.
...
class Logger extends Component
{
    ...
    public function log($message, $level, $category = 'application') {
        ...
        if ($this->traceLevel > 0) {
            ...
            foreach ($ts as $trace) {
                if (isset($trace['file'], $trace['line']) && strpos($trace['file'], Q_PATH) !== 0) {
    ...
    public function flush($final = false) {
        $messages = $this->processMessages( $this->messages );
        $this->messages = [];
	...
	private function processMessages( $messages ) {
		...
		foreach( $messages as $key => $val ) {
			if( $val[2] == 'profile' ) {
				if( $val[1] == self::LEVEL_PROFILE_BEGIN ) {
					$beginProfile[] = $val;
				}
				elseif( $val[1] == self::LEVEL_PROFILE_END ) {
					$endProfile[] = $val;
				}
				unset( $messages[$key] );
			}
		...
		unset( $key, $val );
		foreach( $beginProfile as $key => $val ) {
			if( ( $endendProfiledata = $this->checkArrayIsValue( $val[0],$endProfile ) ) !== false ) {
				$executeTime = round( $endendProfiledata[3]-$val[3], 3 );
				$endendProfiledata[0] = "KEY:{$endendProfiledata[0]} ExecuteTime:{$executeTime}s";
				$endendProfiledata[1] = self::LEVEL_TRACE;
				$endendProfiledata[2] = 'trace';
				$messages[] = $endendProfiledata;
			}
		}
		return $messages;
	}

	private function checkArrayIsValue( $string, $array=[] ) {
     ...
     * defined by constant `Q_BEGIN_TIME` which is evaluated at the beginning
     * of [[\q\BaseQ]] class file.
     * @return float the total elapsed time in seconds for current request.
     */
    public function getElapsedTime() {
        return microtime(true) - Q_BEGIN_TIME;
    }
     ...
     * For example, 'q\db\*' will match categories starting with 'q\db\',
     * such as 'q\db\Connection'.
