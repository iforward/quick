<?php
/**
 * @link http://www.qframework.com/
 * @copyright Copyright (c) 2008 q Software LLC
 * @license http://www.qframework.com/license/
 */
 ...
 *
 * Session is a Web application component that can be accessed via `q::$app->session`.
 *
...
class Session extends Component implements \IteratorAggregate, \ArrayAccess, \Countable
{
	...
	public function init()
	{
		...
		if ($this->getIsActive()) {
			q::warning("Session is already started", __METHOD__);
	...
	public function open()
	{
		...
		if ($this->getIsActive()) {
			q::info('Session started', __METHOD__);
			$this->updateFlashCounters();
		} else {
			$error = error_get_last();
			$message = isset($error['message']) ? $error['message'] : 'Failed to start session.';
			q::error($message, __METHOD__);
		}
	}
	...
	protected function registerSessionHandler()
	{
		if ($this->handler !== null) {
			if (!is_object($this->handler)) {
				$this->handler = q::createObject($this->handler);
			}
	...
	public function regenerateID($deleteOldSession = false)
	{
		...
		// https://github.com/qsoft/q2/pull/1812
	 ...
	 * ```php
	 * <?php
	 * foreach (q::$app->session->getAllFlashes() as $key => $message) {
